...
function UnitTreePage({ match }) {
	...
	function handleTreeClick(event) {
		...
		if (match.params.unittype == "admin") {
			...
			if (event.type == "ogda") {
				...
				history.push(`/useradmineditpage/${ogdaidstr}`);
			}
			if (event.type == "hativa") {
				var hativaidstr = idstr.split("/")[2];
				history.push(`/useradmineditpage/${hativaidstr}`);
			}
			if (event.type == "gdod") {
				var gdodidstr = idstr.split("/")[3];
				history.push(`/useradmineditpage/${gdodidstr}`);
			}
			...
			if (event.type == "gdod") {
				...
				history.push(`/useradmineditpage/gdod/${gdodidstr}`);
			}
	...
	useEffect(() => {
		if (match.params.unittype == "admin") loadAdminData();
		if (match.params.unittype == "pikod") loadPikodData();
		if (match.params.unittype == "ogda") loadOgdaData();
		if (match.params.unittype == "hativa") loadHativaData();
	}, []);
		...
		<Container style={{ paddingTop: "80px" }}>
			<Card>
				<CardHeader style={{ direction: "rtl" }}>
					<CardTitle tag="h4" style={{ direction: "rtl", textAlign: "right" }}>
						עץ יחידות
					</CardTitle>
					{/*headline*/}
				</CardHeader>
